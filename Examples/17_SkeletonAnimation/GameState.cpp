...
void GameState::Initialize()
{
    ...
    ModelManager::Get()->AddAnimation(mCharacter.modelId, "Assets/Models/Character_01/SixSeven.animset");
    ModelManager::Get()->AddAnimation(mCharacter.modelId, "Assets/Models/Character_01/NotFamilyFriendly.animset");
    ...
    mParasite.animator = &mParasiteAnimator;
    ModelManager::Get()->AddAnimation(mParasite.modelId, "Assets/Models/parasite/samba.animset");
    mParasiteAnimator.Initialize(mParasite.modelId);

    // Load zombie
    mZombie.Initialize("zombie/zombie.model");
    mZombie.transform.position = {0.0f, 0.0f, 0.0f};
    mZombie.animator = &mZombieAnimator;
    ModelManager::Get()->AddAnimation(mZombie.modelId, "Assets/Models/zombie/Thriller.animset");
    mZombieAnimator.Initialize(mZombie.modelId);
...
void GameState::Render()
{
    ...
    if (mDrawSkeleton)
    {
        ...
        case CurrentModel::Parasite:
            AnimationUtil::ComputeBoneTransforms(mParasite.modelId, boneTransforms, &mParasiteAnimator);
            AnimationUtil::DrawSkeleton(mParasite.modelId, boneTransforms);
            break;
        case CurrentModel::Zombie:
            AnimationUtil::ComputeBoneTransforms(mZombie.modelId, boneTransforms, &mZombieAnimator);
...
void GameState::DebugUI()
{
    ...
    int maxAnimations = static_cast<int>(mCharacterAnimator.GetAnimationCount());
    int maxParasiteAnimations = static_cast<int>(mParasiteAnimator.GetAnimationCount());
    int maxZombieAnimations = static_cast<int>(mZombieAnimator.GetAnimationCount());
    if (ImGui::DragInt("AnimIndex", &mClipIndex, 1, -1, maxAnimations - 1 | maxParasiteAnimations - 1 | maxZombieAnimations - 1))
    {
