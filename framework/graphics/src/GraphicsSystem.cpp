...
LRESULT GraphicsSystem::GraphicsSystemMessageHandler(HWND window, UINT message, WPARAM wParam,
                                                     LPARAM lParam) {
...
void GraphicsSystem::Initialize(HWND window, bool fullscreen) {
    ...
    HRESULT hr = D3D11CreateDeviceAndSwapChain(
        ...
        &swapChainDesc, &mSwapChain, &mD3DDevice, nullptr, &mImmediateContext);
...
void GraphicsSystem::EndRender() { mSwapChain->Present(mVSync, 0); }
...
void GraphicsSystem::ResetViewport() { mImmediateContext->RSSetViewports(1, &mViewport); }

void GraphicsSystem::SetClearColor(const Color &color) { mClearColor = color; }

void GraphicsSystem::SetVSync(bool vSync) { mVSync = vSync ? 1 : 0; }

uint32_t GraphicsSystem::GetBackBufferWidth() const { return mSwapChainDesc.BufferDesc.Width; }

uint32_t GraphicsSystem::GetBackBufferHeight() const { return mSwapChainDesc.BufferDesc.Height; }
...
ID3D11Device *GraphicsSystem::GetDevice() { return mD3DDevice; }

ID3D11DeviceContext *GraphicsSystem::GetContext() { return mImmediateContext; }