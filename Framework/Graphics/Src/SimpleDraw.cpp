...
void SimpleDrawImpl::AddFace(const Math::Vector3 &v0, const Math::Vector3 &v1,
                             const Math::Vector3 &v2, const Color &color) {
...
void SimpleDraw::AddFace(const Math::Vector3 &v0, const Math::Vector3 &v1, const Math::Vector3 &v2,
                         const Color &color) {
...
void SimpleDraw::AddAABB(float minX, float minY, float minZ, float maxX, float maxY, float maxZ,
                         const Color &color) {
...
void SimpleDraw::AddFilledAABB(float minX, float minY, float minZ, float maxX, float maxY,
                               float maxZ, const Color &color) {
    const Vector3 trf = {maxX, maxY, minZ};
    const Vector3 brf = {maxX, minY, minZ};
    const Vector3 tlf = {minX, maxY, minZ};
    const Vector3 blf = {minX, minY, minZ};
...
void SimpleDraw::AddSphere(uint32_t slices, uint32_t rings, float radius, const Color &color,
    ...
    for (uint32_t r = 0; r < rings; ++r) {
        ...
        for (uint32_t s = 0; s < slices; ++s) {
            ...
            v0 = {radius * sin(rot0) * sin(phi0), radius * cos(phi0),
                  radius * cos(rot0) * sin(phi0)};

            v1 = {radius * sin(rot1) * sin(phi0), radius * cos(phi0),
                  radius * cos(rot1) * sin(phi0)};
            AddLine(v0 + origin, v1 + origin, color);

            v1 = {radius * sin(rot0) * sin(phi1), radius * cos(phi1),
...
void SimpleDraw::AddGroundCircle(uint32_t slices, float radius, const Color &color,
    ...
    for (uint32_t s = 0; s < slices; ++s) {
        ...
        v0 = {radius * sin(rot0), 0.0f, radius * cos(rot0)};
        v1 = {radius * sin(rot1), 0.0f, radius * cos(rot1)};
        AddLine(v0, v1, color);
    }
}
...
void SimpleDraw::Render(const Camera &camera) { sInstance->Render(camera); }
