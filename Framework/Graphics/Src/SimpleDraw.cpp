...
void SimpleDrawImpl::AddFace(const Math::Vector3& v0,
                             const Math::Vector3& v1,
                             const Math::Vector3& v2,
                             const Color& color)
{
...
void SimpleDraw::AddFace(const Math::Vector3& v0,
                         const Math::Vector3& v1,
                         const Math::Vector3& v2,
                         const Color& color)
{
...
void SimpleDraw::AddAABB(float minX,
                         float minY,
                         float minZ,
                         float maxX,
                         float maxY,
...
void SimpleDraw::AddFilledAABB(float minX,
                               float minY,
                               float minZ,
                               float maxX,
                               float maxY,
...
void SimpleDraw::AddSphere(uint32_t slices,
                           uint32_t rings,
                           float radius,
                           const Color& color,
                           const Math::Vector3& origin)
{
    ...
    for (uint32_t r = 0; r < rings; ++r)
    {
        ...
        for (uint32_t s = 0; s < slices; ++s)
        {
            ...
            v0 = {
                radius * sin(rot0) * sin(phi0), radius * cos(phi0), radius * cos(rot0) * sin(phi0)};

            v1 = {
                radius * sin(rot1) * sin(phi0), radius * cos(phi0), radius * cos(rot1) * sin(phi0)};
            AddLine(v0 + origin, v1 + origin, color);

            v1 = {
                radius * sin(rot0) * sin(phi1), radius * cos(phi1), radius * cos(rot0) * sin(phi1)};
            AddLine(v0 + origin, v1 + origin, color);
        }
    }
}
...
void SimpleDraw::AddGroundCircle(uint32_t slices,
                                 float radius,
                                 const Color& color,
                                 const Math::Vector3& origin)
{
